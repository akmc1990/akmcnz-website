      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="font-bold text-church-navy text-lg">오클랜드감리교회 YouTube 채널</h2>
                <p className="text-gray-500 text-sm">구독하고 실시간 예배 알림을 받으세요!</p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              <a href={YOUTUBE_CHANNEL_URL} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white px-5 py-2.5 rounded-lg font-semibold transition-colors">
                <FiYoutube className="w-5 h-5" />
                구독하기
              </a>
              <a href={YOUTUBE_CHANNEL_URL + '?sub_confirmation=1'} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 border-2 border-church-teal text-church-teal hover:bg-church-teal hover:text-white px-5 py-2.5 rounded-lg font-semibold transition-all">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-8">
          <div className="p-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-bold text-church-navy">최근 예배 영상</h2>
            <a href={YOUTUBE_CHANNEL_URL} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-church-teal text-sm hover:underline">
        ...
        <div className="bg-church-cream rounded-xl border border-church-gold/30 p-6">
          ...
          <div className="bg-white rounded-lg p-4 border border-gray-100 text-sm text-gray-600">
            <p className="font-semibold text-church-navy mb-1">인터넷 뱅킹 레퍼런스 입력 방법</p>
            <p>헌금코드 + 영문이름 형식으로 입력해 주세요.</p>
            <p className="mt-1 text-church-teal font-medium">예: 주일헌금 홍길동 → <span className="font-bold">2Hgildong</span></p>
          </div>
        </div>
      </div>
    </div>

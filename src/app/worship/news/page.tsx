        ...
        {isAdmin && showUpload && (
          <div className="bg-white rounded-xl shadow-md p-6 mb-8 border border-gray-200">
            ...
            <form onSubmit={handleUpload} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-sm font-semibold text-gray-600">.jsx 파일 선택 *</label>
                <input
                  type="file"
                  accept=".jsx,.js"
                ...
                />
                <p className="text-xs text-gray-400">카드뉴스 .jsx 파일을 선택하세요. 파일 내에 const cards = [...] 배열이 있어야 합니다.</p>
              </div>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '28px', flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
        {c.preacher && <div style={{ background: C.goldBg, borderRadius: 14, padding: '14px 18px', border: '1.5px solid ' + C.border }}><div style={{ color: C.muted, fontSize: 12, fontWeight: 600, marginBottom: 4 }}>설교자</div><div style={{ color: C.brownDark, fontSize: 18, fontWeight: 700 }}>{c.preacher}</div></div>}
        {c.scripture && <div style={{ background: C.sky, border: '1.5px solid ' + C.skyBorder, borderRadius: 14, padding: '14px 18px' }}><div style={{ color: '#1E4DAA', fontSize: 13, fontWeight: 600, marginBottom: 4 }}>본문</div><div style={{ color: '#1E4DAA', fontSize: 16, fontWeight: 600 }}>{c.scripture}</div></div>}
        {c.items && <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>{c.items.map((it, i) => <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid ' + C.border }}><span style={{ color: C.muted, fontSize: 14 }}>{it.label}</span><span style={{ color: C.brownDark, fontSize: 14, fontWeight: 600 }}>{it.value}</span></div>)}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '28px', flex: 1 }}>
        {c.items && <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>{c.items.map((it, i) => <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: '1px solid ' + C.border }}>{it.time && <span style={{ color: C.muted, fontSize: 12, minWidth: 40 }}>{it.time}</span>}<span style={{ color: C.brownDark, fontSize: 15, flex: 1 }}>{it.name}</span>{it.person && <span style={{ color: C.muted, fontSize: 13 }}>{it.person}</span>}</div>)}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '28px', flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {c.items && <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>{c.items.map((it, i) => <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 14px', background: C.goldBg, borderRadius: 12, border: '1px solid ' + C.border }}><span style={{ color: C.muted, fontSize: 14 }}>{it.label}</span><span style={{ color: C.brownDark, fontSize: 15, fontWeight: 700 }}>{it.value}명</span></div>)}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '28px', flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
        {c.items && <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>{c.items.map((it, i) => <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', background: i % 2 === 0 ? C.goldBg : C.green, borderRadius: 12, border: '1px solid ' + (i % 2 === 0 ? C.border : C.greenBorder) }}><span style={{ color: C.muted, fontSize: 14 }}>{it.label}</span><span style={{ color: C.brownDark, fontSize: 15, fontWeight: 700 }}>{it.value}</span></div>)}</div>}
        {c.note && <div style={{ marginTop: 8, color: C.muted, fontSize: 13, textAlign: 'center' }}>{c.note}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'linear-gradient(160deg,' + C.goldBg + ' 0%,#fff 100%)' }}>
      <div style={{ padding: '36px 28px', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, textAlign: 'center' }}>
        ...
        {c.message && <div style={{ color: C.brownDark, fontSize: 20, fontWeight: 800, lineHeight: 1.5 }}>{c.message}</div>}
        {c.verse && <div style={{ background: C.goldBg, borderLeft: '4px solid ' + C.gold, borderRadius: '0 12px 12px 0', padding: '16px 20px', color: C.brown, fontSize: 15, lineHeight: 1.9, textAlign: 'left', width: '100%' }}>{c.verse}</div>}
        {c.ref && <div style={{ color: C.muted, fontSize: 14, fontWeight: 600 }}>— {c.ref}</div>}
